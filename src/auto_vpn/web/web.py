...
class VPNApplication:
    ...
    THRESHOLD_OPTIONS: ClassVar[dict[str, timedelta]] = {
    ...
    def __init__(self):
        st.set_page_config(page_title="VPN Manager", page_icon="🔒", layout="wide")
    ...
    def _init_monitoring(self):
        ...
        if (
            "periodic_task" not in st.session_state
            or not st.session_state.periodic_task.running
        ):
    ...
    def _init_self_ping(self):
        ...
        if (
            "periodic_task_ping" not in st.session_state
            or not st.session_state.periodic_task_ping.running
        ):
            ...
            def ping_self():
                try:
                    ...
                    logger.info(
                        f"Self-ping completed with status {response.status_code}"
                    )
    ...
    def _render_peer_details(self, peer: dict):
        ...
        st.write(
            f"{peer['country_flag']} {peer['country']} | 🖥️ {peer['ip_address']} | ⏱️ {peer['age']}"
        )
    ...
    def _render_delete_button(self, peer: dict):
        ...
        if st.button(
            "🗑️ Delete", key=f"delete_{peer['peer_id']}", use_container_width=True
        ):
            try:
                ...
                st.error(f"Error deleting peer: {e!s}")
    ...
    def _get_current_threshold_info(self):
        ...
        current_label = next(
            (
                label
                for label, td in self.THRESHOLD_OPTIONS.items()
    ...
    def _render_threshold_settings(self):
        ...
        if (
            "threshold_changed" in st.session_state
            and st.session_state.threshold_changed
        ):
    ...
    def _handle_vpn_creation(self, location: str):
        ...
        try:
            with st.spinner("Creating VPN peer..."):
                ...
                st.session_state.vpn_peers = (
                    st.session_state.vpn_manager.refresh_peers()
                )

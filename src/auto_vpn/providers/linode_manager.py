...
from auto_vpn.core.utils import generate_password
...
class LinodeManager(InfrastructureManager):
    ...
    def pulumi_program(self):
        ...
        ssh_key = linode.SshKey(f"{self.project_name}-ssh-key",
        ...
        instance = linode.Instance(f"{self.project_name}-vpn",
    ...
    def set_stack_config(self):
        ...
        """
        config_settings = {
            "linode:token": auto.ConfigValue(
                value=self.linode_api_key,
                secret=True
            ),
        }
